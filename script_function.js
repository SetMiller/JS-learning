...
// на этот объект с помощью ключевого слова this
  ...
  // Вариант 4 -> стрелочная функция не имеет собственного this, а использует this функции в которой была вызвана
  function trust5(){ 
...
// Наследует некоторые методы и свойства через цепочку прототипов объекта Function.prototype.
  ...
  function constructFunction() {
    let scope3 = "local";
    function f(){return scope3};                        
    return f();
  };
  constructFunction();                                   //👈 scope3 => "local"
  ...
  constructFunction();                                  //👈 scope3 => "nested scope"                                   
  ...
  // Вариант 4 - Замыкание + выражение определения самовызывающейся функции
  let nest = (function constructFunction() {
    let scope3 = "local";
    function f(){
      let scope3 = "nested scope";
      function nested(){
...
// При объявлении функции необязательные аргументы должны завершать список аргументов, чтобы их можно было опустить
  ...
  function getPropertyNames2 (obj, /* необязательный */ arr){
    arr = arr || [];
    for(prop in obj) {
      if(typeof obj[prop] === 'function') continue;       //👈 фильтруем методы
      arr.push(prop);
    };
    return arr;
  };
...
// Arguments - это объект, подобный массиву, позволяющий извлекать переданные функции значения по их номерам, а не по именам
  ...
  // Копируем получаемые аргументы в массив
  function arg(){
    let arr = [];
    for(prop in arguments) arr.push(arguments[prop]);
    return arr;
  };

  // Косвенно применяем метод массива к объекту подобному массиву
  function arg(){
    return Array.prototype.map.call(arguments, function(itemValue){return itemValue});
  };

  let arr9 = arg(1,2,3,'w');                                    //👈 => [1, 2, 3, "w"]
...
// Принимает любое число входных аргументов и рекурсивно обрабатывает те из них, которые являются массивами
  function flexSum(a) {
    let total = 0;
...
// 2️⃣it has access to the outer function’s variables, and 3️⃣it has access to the global variables.
  ...
  function checkScope() {
    ...
    // Пара круглых скобок перемещена из тела функции за еепределы.
    // Вместо вызова вложенной функции и возврата ее результата функция, теперь просто возвращает сам объект вложенной функции🔥🔥🔥
    return f;
  }
  ...
  // Пример 6.
  let count2 =  function integerCounter3(arg, numb){
                  function count(j, k){
                    let counter = k;
                    return j+counter;
                  }(arg, numb);
...
// Пример
  ...
  function checkArgs(x, y, z){
    console.log(arguments.length);                      //👈 -> определение переданного количества аргументов
    console.log(arguments.callee.length);               //👈 -> 3, определение ожидаемого количества аргументов
  };

// 2️⃣ Свойство prototype
// Любая функция имеет свойство prototype, ссылающееся на объект, известный как объект прототипа.
...
// When the apply method is called on an object func with arguments thisArg and argArray
  ...
  // o.m(10);
  // delete o.m;
  let objEx = {x: 10, y: 20, z:30};
  function sum(){return this.x + this.y + this.z};
  let result = sum.call(objEx);                         
  result;                                                   //👈 -> 10 + 20 + 30 => 60

  function sum2(z, k){return this.x + this.y + z + k};
  let result2 = sum2.call(objEx, 10, 5);                
  result2;                                                  //👈 -> 10 + 20 + 10 + 5 => 45
                                                      
  let result3 = sum2.apply(obj,[10, 5])
  result3;                                                  //👈 -> 10 + 20 + 10 + 5 => 45

  let result3 = sum2.bind(obj);
  result3(5, 10)                                            //👈 -> передать значения "z" и "k" -> 10 + 20 + 5 + 10 => 45
...
// значение - соответствующим объектом-оберткой
  ...
  function trace(o, m) {
    ...
    let original = o[m];
    // определить новый функционал метода
    o[m] = function() {
...
// Вызов новой функции выполнит вызов оригинальной функции f4() как метода объекта "о"💎💎💎
  ...
  let g5 = bind2(f4, o3);
  let at3 = bind2(sum2, obj);
  at3(5,6)                                //👈 -> передать значения "z" и "k" -> 10 + 20 + 5 + 10 => 45 (строка 1010)
